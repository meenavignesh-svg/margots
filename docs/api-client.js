(() => {
  ...
  // Provider keys live only in this JavaScript module's memory. They are never
  // written to localStorage/sessionStorage/cookies/URLs and are never sent to
  // the MARGOTS backend. Reloading or closing the page clears them.
  ...
  function clearKey(provider) {
    ...
    else Object.keys(keys).forEach(k => delete keys[k]);
  }
  ...
  async function providerChat(provider, messages, model) {
    ...
    try {
      const response = await fetch(cfg.endpoint, {
        ...
        body: JSON.stringify(cfg.body(model || cfg.defaultModel, messages)),
        signal: controller.signal,
        cache: 'no-store',
        credentials: 'omit'
      });
      let data = null;
      try { data = await response.json(); } catch { throw new Error(`AI provider returned invalid JSON (HTTP ${response.status}).`); }
      if (!response.ok) {
        const message = data?.error?.message || data?.message || `AI provider request failed (HTTP ${response.status}).`;
  ...
  async function backendRequest(path, options = {}) {
    if (!navigator.onLine) throw Object.assign(new Error('MARGOTS is offline. Reconnect for deterministic backend analysis.'), { code: 'OFFLINE' });
    const controller = new AbortController();
    ...
    try {
      ...
      let payload;
      try { payload = await response.json(); } catch { throw new Error(`MARGOTS backend returned invalid JSON (HTTP ${response.status}).`); }
      if (!response.ok || payload?.success === false) {
        const e = payload?.error;
        throw Object.assign(new Error(e?.message || `Backend request failed (HTTP ${response.status}).`), { code: e?.code || `HTTP_${response.status}`, status: response.status });
      }
      return payload;
    } catch (error) {
  ...
  function buildPrompt(request, deterministic) {
    return [
      'You are MARGOTS, a careful scientific bioinformatics assistant.',
      'Use the deterministic analysis below as the primary factual input. Do not invent experimental results, database evidence, citations, clinical conclusions, or measurements.',
      'Clearly distinguish calculation, interpretation, hypothesis, and evidence that would be needed to verify a claim.',
      `TASK: ${request.question || request.context || request.variant || 'Interpret the supplied biological analysis.'}`,
      `MODE: ${request.mode}`,
      `INPUT: ${request.sequence || request.variant || request.text || request.context || ''}`,
      `DETERMINISTIC ANALYSIS: ${JSON.stringify(deterministic || {}, null, 2)}`
    ].join('\n\n');
  }

  async function analyzeWithAI(provider, model, request, deterministic) {
    const messages = [
      { role: 'system', content: 'Return concise, scientifically cautious Markdown. Never claim that an AI interpretation is experimental or clinical validation.' },
      { role: 'user', content: buildPrompt(request, deterministic) }
    ];
    return providerChat(provider, messages, model);
  }

  // Public API: no function accepts a provider key as a backend request parameter.
  window.MargotsAPI = {
  ...
  // Session-only means a full page lifecycle is enough; this is an additional
  // cleanup hook and does not persist or transmit the key.
  ...
  const theme = document.createElement('meta'); theme.name = 'theme-color'; theme.content = '#111827'; document.head.appendChild(theme);
